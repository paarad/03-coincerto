...
export function buildOverlaySVG(opts: {
  ...
  // Bandeau bas - scaled up for 1024x1024
  const bandH = 184; // Doubled from 92
  const bandY = height - bandH;

  // Sparkline - scaled up for 1024x1024
  const sparkW = 480; // Doubled from 240
  const sparkH = 112; // Doubled from 56
  const sparkX = width - sparkW - 32; // Doubled margin
  const sparkY = bandY + 36; // Doubled from 18
  const path = sparklinePath(prices, sparkW, sparkH, 12); // Doubled padding

  // Badge Fear&Greed - scaled up for 1024x1024
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Titre -->
  <text x="48" y="${bandY + 72}" font-family="Inter, Arial, sans-serif" font-weight="700"
        font-size="56" fill="white">${escapeXml(title)}</text>

  <!-- Date -->
  <text x="48" y="${bandY + 144}" font-family="Inter, Arial, sans-serif"
        font-size="40" fill="white" opacity="0.9">${escapeXml(date)}</text>
  ...
  ${
    ...
    <g transform="translate(${sparkX}, ${sparkY})">
      <rect x="0" y="0" width="${sparkW}" height="${sparkH}" rx="16" ry="16" fill="white" fill-opacity="0.06" />
      <path d="${path}" fill="none" stroke="white" stroke-width="5" stroke-linejoin="round" stroke-linecap="round"/>
    </g>
  `
      : ``
  }
  ...
  ${
    fg !== undefined
      ? `
    <g transform="translate(${width - 48 - 160}, ${bandY + 108})">
      <rect x="0" y="0" width="160" height="48" rx="24" fill="${fgColor}" />
      <text x="80" y="32" font-size="24" text-anchor="middle" font-family="Inter, Arial, sans-serif" fill="white">
        F&amp;G ${fg}
      </text>
    </g>
  `
...
export async function applyOverlay(options: {
  ...
  // Redimensionner à 1024x1024 pour optimiser la taille
  const resized = base.resize(1024, 1024, { 
    fit: 'cover',
    position: 'center'
  });

  const overlay = buildOverlaySVG({ title, date, width: 1024, height: 1024, prices, fearGreed });
